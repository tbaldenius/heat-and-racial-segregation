...
Rcpp::sourceCpp("Code/9c_helper_conley.cpp")
...
conley_ses <- function(X, e, coords, dist_cutoff, lag_cutoff = 0, id = NULL, time = NULL, kernel = "bartlett", W = 1, dist_fn = "haversine", K = NULL) {
	...
	if(panel) {
		...
		XeeXhs <- lapply(time_unique, function(t) {
			...
			XeeX_spatial(coords=sub_coords, cutoff=dist_cutoff, X=sub_X, e=sub_e, k=k, kernel=kernel, dist_fn = dist_fn)
		})
		
		# Reduce across time
		XeeXh_spatial <- Reduce("+",  XeeXhs)
	} else {
		XeeXh_spatial <- XeeX_spatial(coords=coords, cutoff=dist_cutoff, X=X, e=e, k=k, kernel=kernel, dist_fn = dist_fn)
	}
	...
	if(panel) {
	  XeeXh_serial <- XeeX_serial(time=time, id=id, cutoff=lag_cutoff, X=X, e=e, k=k)
	  if(any(diag(XeeXh_serial) < 0)) warning("HAC VCOV contains negative values.")
	}
	...
	# SSC robust SEs
	V_robust <-  (n-1)/(n-K) * (invXX %*% XeeXh_robust %*% invXX)
